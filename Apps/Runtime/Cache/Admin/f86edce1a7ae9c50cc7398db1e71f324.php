    ...
    <div id="Member_Ratings">
        <div class="d_Confirm_Order_style">
            <div class="search_style">
                ...
                <form action="/admin.php/Badge" method="get">
                    <ul class="search_content clearfix">
                        <li><label class="l_f">搜索查询：</label><input name="keyword" id="keyword" type="text" value="<?php echo ($keyword); ?>" class="text_add" placeholder="输入徽章名称"  style=" width:400px"/></li>
                       <!-- <li><label class="l_f">排序依据：</label></li>
                        ...
                        -->
                        <!--
                                                <li style="width:90px;"><a href="/admin.php/Badge/index/type/0" class="btn btn-danger" style="padding:0px 10px;">重置</a></li>
                        -->
                    </ul>
            ...
            <div class="border clearfix" style="margin-top:20px;">
				<span class="l_f">
					<a href="/admin.php/Badge/postbadge"  id="cate_add" class="btn btn-warning"><i class="icon-plus"></i>新建</a>
            ...
            <div class="table_menu_list" id="imglist">
                <table class="table table-striped table-bordered table-hover" id="sample-table">
                    ...
                    <tr>
                        ...
                        <th width="150">封面</th>
                        <th width="200">名称</th>
                        <th width="200">规则说明</th>
                        <th width="150">获取人数</th>
                        <th width="200">
                            操作
                        </th>
                    </tr>
                    ...
                    <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "$empty" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                            ...
                            <td><img src="<?php echo ($vo["cover"]); ?>" style="height:50px;"/></td>
                            <td><?php echo ($vo["name"]); ?></td>
                            <td><?php echo ($vo["rule_content"]); ?></td>
                            <td>
                                <?php echo ($vo["count"]); ?>
                                <a title="详情"  href="/admin.php/Badge/detail/id/<?php echo ($vo["id"]); ?>"  class="btn btn-xs btn-success" >详情</a>
                            </td>
                            <td>
                                <a title="编辑"  href="/admin.php/Badge/postbadge/id/<?php echo ($vo["id"]); ?>"  class="btn btn-xs btn-info" >编辑</a>
...
<script>
    ...
    function edit($id){
        ...
        layer.open({
            ...
            yes:function(index,layero){
                ...
                $.post('/admin.php/Badge/savecommentreply',{commentid:commentid,content:content},function(data){
                    /*data是返回的值*/
                    console.log(data)

                    if(data.status==1){
                        layer.alert('成功！',{title: '提示框',icon:1},function(){
                            window.location.href="/admin.php/Badge/comment";
                        ...
                        layer.alert('失败！',{title: '提示框',icon:2},function(){
                            window.location.href="/admin.php/Badge/comment"; // 父页面刷新
                            var index = parent.layer.getFrameIndex(window.name); //获取窗口索引
                            parent.layer.close(index);
                        });
                    }
    ...
    function deleted($id){
        ...
        $.post('/admin.php/Badge/delete',{id:id},function(data){
            /*data是返回的值*/
            console.log(data)

            if(data.status==1){
                layer.alert('成功！',{title: '提示框',icon:1},function(){
                    window.location.href="/admin.php/Badge/index";
                    //parent.location.reload(); // 父页面刷新
                    var index = parent.layer.getFrameIndex(window.name); //获取窗口索引
                    parent.layer.close(index);
                });
                ...
                layer.alert('失败！',{title: '提示框',icon:2},function(){
                    window.location.href="/admin.php/Badge/index"; // 父页面刷新
                    var index = parent.layer.getFrameIndex(window.name); //获取窗口索引
                    parent.layer.close(index);
                });
            }
