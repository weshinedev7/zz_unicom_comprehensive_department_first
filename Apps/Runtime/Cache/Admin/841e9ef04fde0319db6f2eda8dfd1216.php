    ...
    <div id="Member_Ratings">
        <div class="d_Confirm_Order_style">
            ...
            <div class="search_style">
                ...
                <form action="/admin.php/Rebook/index" method="get">
                    <ul class="search_content clearfix">
                        <li><label class="l_f">搜索查询：</label><input name="keyword" id="keyword" type="text" value="<?php echo ($keyword); ?>" class="text_add" placeholder="输入预约内容"  style=" width:400px"/></li>
                        ...
                        -->
                        <!--
                                                <li style="width:90px;"><a href="/admin.php/Rebook/index/type/0" class="btn btn-danger" style="padding:0px 10px;">重置</a></li>
                        -->
                    </ul>
            ...
            <!--<div class="border clearfix" style="margin-top:20px;">-->
				<!--<span class="l_f">-->
					<!--<a href="/admin.php/Rebook/posttraffic"  id="cate_add" class="btn btn-warning"><i class="icon-plus"></i>新建</a>-->
            ...
            <div class="table_menu_list" id="imglist">
                <table class="table table-striped table-bordered table-hover" id="sample-table">
                    ...
                    <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "$empty" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                            ...
                            <td><?php echo ($vo["rebooktime"]); ?></td>

                            <td>
                                <!--<a title=""  href="/admin.php/Rebook/posttraffic/id/<?php echo ($vo["id"]); ?>"  class="btn btn-xs btn-info" >编辑</a>-->
...
<script>
    function deleted($id){
        ...
        if($con==true){
            $.post('/admin.php/Rebook/deleterebook',{id:$id},function(data){
                if(data.status>0){
                    layer.alert('成功！',{title: '提示框',icon:1},function(){
                        window.location.href="/admin.php/Rebook/index";
                    ...
                    layer.alert('失败！',{title: '提示框',icon:2},function(){
                        window.location.href="/admin.php/Rebook/index"; // 父页面刷新
                        var index = parent.layer.getFrameIndex(window.name); //获取窗口索引
                        parent.layer.close(index);
                    });
                }
    ...
    function edit($id){
        ...
        $('#tagid').val('')
        if ($id >0){
            $.post('/admin.php/Rebook/gettaginfo',{id:$id},function(data){
        ...
        layer.open({
            ...
            yes:function(index,layero){
                ...
                var id=$('#tagid').val()
                $.post('/admin.php/Rebook/edittag',{tagname:tagname,id:id},function(data){
                    /*data是返回的值*/
                    if(data.status>0){
                        layer.alert('成功！',{title: '提示框',icon:1},function(){
                            window.location.href="/admin.php/Rebook/tags";
                            //parent.location.reload(); // 父页面刷新
                            var index = parent.layer.getFrameIndex(window.name); //获取窗口索引
                            parent.layer.close(index);
                        });
                        ...
                        layer.alert('失败！',{title: '提示框',icon:2},function(){
                            window.location.href="/admin.php/Rebook/tags"; // 父页面刷新
                            var index = parent.layer.getFrameIndex(window.name); //获取窗口索引
                            parent.layer.close(index);
                        });
                    }
