...
<div class="clearfix" id="add_picture">
    <div class="page_right_style" style="left:0px;width: 100%;">
        ...
        <div class="form form-horizontal" id="form-article-add">
            ...
            <div class="clearfix cl">
                ...
                <div class="col-lg-8 col-md-8" >
                    <select id="catid" name='catid' >
                        <?php if(is_array($cate)): $i = 0; $__LIST__ = $cate;if( count($__LIST__)==0 ) : echo "$empty" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if($vo['id'] == $data['catid']): ?><option value="<?php echo ($vo["id"]); ?>" selected="selected"><?php echo ($vo["name"]); ?></option>
                                <?php else: ?>
            ...
            <div class="clearfix cl">
                <label class="col-lg-2 col-md-2 col-sm-12 control-label"><span class="c-red" style="color:red;">*&nbsp;&nbsp;</span>常用电话：</label>
                <div class="col-lg-8 col-md-8" >
                    <input type="text" class="input-text" value="<?php echo ($data["tel"]); ?>" placeholder="" name="tel" id="tel">
                </div>
            </div>
            <!--<div class="clearfix cl">-->
            ...
            <!--<img src="https://quanyu.xcxweshine.com/<?php echo ($data["cover"]); ?>" id="coverimg" width="370px" height="146" style="border:1px solid lightgrey" alt="">-->
            <!--</div>-->
            <!--</div>-->
            ...
            <div class="clearfix cl">
                <label class="col-lg-2 col-md-2 col-sm-12 control-label"><span class="c-red" style="color:red;">*&nbsp;&nbsp;</span>地图定位：</label>
                <div class="col-lg-8 col-md-8 ">
                    <input type="text" class="form-control" name="mapword" id="mapword" value="<?php echo ($data["mapword"]); ?>" style="margin-bottom:10px;" style="width: 60%;">
                    <button onclick="searchmapkeyword()" id="mapsearch" class="btn btn-success" style="padding:0px 10px;">查询</button>
                    <button onclick="setcoordinate()" id="setmap" class="btn btn-info" style="padding:0px 10px;">确认使用该位置</button>
                </div>
            </div>
            <div class="clearfix cl">
                <label class="col-lg-2 col-md-2 col-sm-12 control-label"><span class="c-red" style="color:red;">*&nbsp;&nbsp;</span></label>
                <div class="col-lg-8 col-md-8 ">
                    <div id="container">
            ...
            <div class="clearfix cl">
                <label class="col-lg-2 col-md-2 col-sm-12 control-label"><span class="c-red" style="color:red;">*&nbsp;&nbsp;</span>地图坐标：</label>
                <div class="col-lg-8 col-md-8" >
                    经度 : <input type="text" class="input-text" value="<?php echo ($data["lat"]); ?>" placeholder="" name="lat" id="lat" style="width: 30%;">
                    纬度 : <input type="text" class="input-text" value="<?php echo ($data["lng"]); ?>" placeholder="" name="lng" id="lng" style="width: 30%;">
                </div>
            </div>
            ...
            <!--<input type="text" class="form-control" name="audio" id="audio" value="<?php echo ($audio["filepath"]); ?>" style="margin-bottom:10px;">-->
            ...
            <!--<input type="text" class="form-control" name="video" id="video" value="<?php echo ($data["videopath"]); ?>" style="margin-bottom:10px;">-->
            <!--</div>-->
            <!--<div class="col-lg-1 col-md-1">-->
...
<script charset="utf-8" src="https://map.qq.com/api/js?v=2.exp&key=2WRBZ-XLVKQ-EBB5I-GMVFJ-REH2E-FIF2M"></script>
<script>
    var searchService,map,markers = [];
    var locationlat=$('#lat').val();
    var locationlng=$('#lng').val();
    var markers='';
    window.onload = function(){
        //初始化地图函数  自定义函数名init
        function init() {
            //定义map变量 调用 qq.maps.Map() 构造函数   获取地图显示容器
            createmap(locationlat,locationlng)
            searchService = new qq.maps.SearchService({
                complete : function(results){
                    console.log(results);
                    var pois = results.detail.pois;
                    if(pois==undefined){
                        alert('关键字太过宽泛!请重新搜索,例:加上城市名')
                    }else{
                        //获取搜索结果的第一个.创建可移动的点位
                        createmap(pois[0].latLng.lat,pois[0].latLng.lng);
                    }
    ...
    function setcoordinate(){
        var pointposition=markers.position;
        $('#lat').val(pointposition.lat)
        $('#lng').val(pointposition.lng)
        console.log(markers.position);
    }
    function createmap(locationlat,locationlng){
        var center = new qq.maps.LatLng(locationlat,locationlng);
    ...
    function searchmapkeyword(){
        var keyword = document.getElementById("mapword").value;
...
<script>
    ...
    var Check = {
        ...
        checkSubmit : function() {
            ...
            if (tel==''){
                layer.msg("请填写常用电话！",{icon:2,time:1000},function(){});
                return false;
            }
            // if(cover==''){
            ...
            if (mapword==''){
                layer.msg("请上传地图位置！",{icon:2,time:1000},function(){});
                return false;
            }
            var data = {
                id:id,
            ...
            $.ajax(
                {
                    url:'/admin.php/Convenience/saveconvenience',
                    data:data,
                    type:'POST',
                    dataType:'json',
                    success:function(json){
                        ...
                        if(json.flag==1){
                            layer.msg('成功!',{icon:1,time:2000},function(){
                                window.location.href="/admin.php/Convenience";
