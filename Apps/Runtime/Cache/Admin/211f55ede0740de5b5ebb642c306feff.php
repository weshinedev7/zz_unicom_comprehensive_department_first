    ...
    <div id="Member_Ratings">
        <div class="d_Confirm_Order_style">
            <div class="border clearfix" style="margin-top:20px;">
				<span class="l_f">
					<a href="javascript:ovid()" onclick="edit()" id="cate_add" class="btn btn-warning"><i class="icon-plus"></i>添加用户标签</a>
...
<script>
    function deleted($id){
        ...
        if($con==true){
            $.post('/admin.php/User/deletetag',{id:$id},function(data){
                if(data.status>0){
                    layer.alert('成功！',{title: '提示框',icon:1},function(){
                        window.location.href="/admin.php/User/tags";
                    ...
                    layer.alert('失败！',{title: '提示框',icon:2},function(){
                        window.location.href="/admin.php/User/tags"; // 父页面刷新
                        var index = parent.layer.getFrameIndex(window.name); //获取窗口索引
                        parent.layer.close(index);
                    });
                }
    ...
    function edit($id){
        ...
        $('#tagid').val('')
        if ($id >0){
            $.post('/admin.php/User/gettaginfo',{id:$id},function(data){
        ...
        layer.open({
            ...
            yes:function(index,layero){
                ...
                var id=$('#tagid').val()
                $.post('/admin.php/User/edittag',{tagname:tagname,id:id},function(data){
                    /*data是返回的值*/
                    if(data.status>0){
                        layer.alert('成功！',{title: '提示框',icon:1},function(){
                            window.location.href="/admin.php/User/tags";
