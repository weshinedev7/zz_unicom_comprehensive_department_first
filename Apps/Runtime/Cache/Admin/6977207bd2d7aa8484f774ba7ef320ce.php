...
<script>
    function deleted($id){
        ...
        if($con==true){
            $.post('/admin.php/Badge/deletecate',{id:$id},function(data){
                if(data.status>0){
                    layer.alert('成功！',{title: '提示框',icon:1},function(){
                        window.location.href="/admin.php/Badge/category";
                    ...
                    layer.alert('失败！',{title: '提示框',icon:2},function(){
                        window.location.href="/admin.php/Badge/category"; // 父页面刷新
                        var index = parent.layer.getFrameIndex(window.name); //获取窗口索引
                        parent.layer.close(index);
                    });
                }
    ...
    function edit($id){
        ...
        $('#tagid').val('')
        if ($id >0){
            $.post('/admin.php/Badge/getcateinfo',{id:$id},function(data){
        ...
        layer.open({
            ...
            yes:function(index,layero){
                ...
                var id=$('#tagid').val()
                $.post('/admin.php/Badge/editcate',{tagname:tagname,id:id},function(data){
                    /*data是返回的值*/
                    if(data.status>0){
                        layer.alert('成功！',{title: '提示框',icon:1},function(){
                            window.location.href="/admin.php/Badge/category";
