...
class RecommendController extends Controller{
    ...
    public function index(){
        $_GPC=$this->_GPC;
        $Model=new Model();
        $sql="select `id`,`name`,`cover`,`time` from wx_article where type=5  ";
        $condition='';
        $keyword='';
        ...
        $sql="select `id`,`name`,`cover`,`time` from wx_article where type=5 {$condition}  order by id desc  limit {$firstRow},{$listRows} ";
    ...
    public function postrecommend(){
        $_GPC=$this->_GPC;
        $id=trim($_GPC['id']);
        $cate=M('category')->where('type=5')->select();

        if (!empty($id)){
            $recommend=M('article')->where(array('id'=>$id))->find();
            $this->assign('id',$id);
