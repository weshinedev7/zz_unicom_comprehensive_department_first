...
class ExpressionController extends Controller{
    ...
    public function index(){
        $_GPC=$this->_GPC;
        $Model=new Model();
        $sql="select `id`,`name`,`cover`,`time` from wx_article where type=1  ";
        $condition='';
        $keyword='';
        ...
        $sql="select `id`,`name`,`cover`,`time` from wx_article where type=1 {$condition}  order by id desc  limit {$firstRow},{$listRows} ";
    ...
    public function postexpression(){
        $_GPC=$this->_GPC;
        $id=trim($_GPC['id']);
        $cate=M('category')->where('type=1')->select();

        if (!empty($id)){
            $expression=M('article')->where(array('id'=>$id))->find();
            $this->assign('id',$id);
