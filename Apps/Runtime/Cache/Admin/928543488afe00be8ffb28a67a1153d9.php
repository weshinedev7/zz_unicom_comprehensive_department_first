    ...
    <div id="Member_Ratings">
        <div class="d_Confirm_Order_style">
            <div class="border clearfix" style="margin-top:20px;">
				<span class="l_f">
					<a href="javascript:ovid()" onclick="edit()" id="cate_add" class="btn btn-warning"><i class="icon-plus"></i>添加热门标签</a>
...
<div class="add_menber" id="add_style" style="display:none">
    <ul class=" page-content">
        <li class="adderss">
            <label class="label_name"> 标签：</label>
...
<script>
    function deleted($id){
        $con=confirm('确认要删除这个标签吗?');
        if($con==true){
            $.post('/admin.php/Hottags/deletetag',{id:$id},function(data){
                if(data.status>0){
                    layer.alert('成功！',{title: '提示框',icon:1},function(){
                        window.location.href="/admin.php/Hottags";
                    ...
                    layer.alert('失败！',{title: '提示框',icon:2},function(){
                        window.location.href="/admin.php/Hottags"; // 父页面刷新
                        var index = parent.layer.getFrameIndex(window.name); //获取窗口索引
                        parent.layer.close(index);
                    });
                }
    ...
    function edit($id){
        ...
        $('#tagid').val('')
        if ($id >0){
            $.post('/admin.php/Hottags/gettaginfo',{id:$id},function(data){
        ...
        layer.open({
            ...
            yes:function(index,layero){
                ...
                var id=$('#tagid').val()
                $.post('/admin.php/Hottags/edittag',{tagname:tagname,id:id},function(data){
                    /*data是返回的值*/
                    if(data.status>0){
                        layer.alert('成功！',{title: '提示框',icon:1},function(){
                            window.location.href="/admin.php/Hottags";
