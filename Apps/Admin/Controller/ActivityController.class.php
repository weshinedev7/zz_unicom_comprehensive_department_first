...
class ActivityController extends Controller{
    ...
    public function index(){
        $_GPC=$this->_GPC;
        $Model=new Model();
        $sql="select `id`,`name`,`cover`,`time` from wx_article where type=4  ";
        $condition='';
        $keyword='';
        ...
        $sql="select `id`,`name`,`cover`,`time` from wx_article where type=4 {$condition}  order by id desc  limit {$firstRow},{$listRows} ";
    ...
    public function postactivity(){
        $_GPC=$this->_GPC;
        $id=trim($_GPC['id']);
        $cate=M('category')->where('type=4')->select();
        if (!empty($id)){
            $activity=M('article')->where(array('id'=>$id))->find();
            $this->assign('id',$id);
